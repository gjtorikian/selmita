...
Rake::ExtensionTask.new("selmita", SELMITA_SPEC) do |ext|
  ext.source_pattern = "*.{rs,toml}"

  ext.lib_dir = File.join("lib", "selmita")
  ...
  CROSS_RUBIES.find_all { |cr| cr.windows? || cr.linux? || cr.darwin? }.map(&:platform).uniq.each do |platform|
    ...
    namespace platform do
      ...
      task "builder" do
        ...
        puts "Invoking #{"pkg/#{SELMITA_SPEC.full_name}-#{Gem::Platform.new(platform)}.gem"}  ..."

        Rake::Task["pkg/#{SELMITA_SPEC.full_name}-#{Gem::Platform.new(platform)}.gem"].invoke
      end
    end
  end
